...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /* ── BLOG LIVE SEARCH & TAG FILTERING ── */
    ...
    function updateBlogDisplay() {
        ...
        blogCards.forEach((card, index) => {
            ...
            const matchesCategory = (currentFilter === 'all' || category === currentFilter || textContent.includes(currentFilter.toLowerCase()));
            const matchesSearch = (!searchQuery || textContent.includes(searchQuery.toLowerCase()));

            if (matchesCategory && matchesSearch) {
                visibleCount++;
                if (currentFilter === 'all' && !searchQuery && visibleCount > 6 && loadMoreBtn && loadMoreBtn.style.display !== 'none') {
    ...
    // Handle Tag Cloud Pills
    tagPills.forEach(pill => {
        pill.addEventListener('click', (e) => {
            e.preventDefault();
            const filter = pill.getAttribute('data-filter') || pill.innerText.replace('#', '').toLowerCase();
